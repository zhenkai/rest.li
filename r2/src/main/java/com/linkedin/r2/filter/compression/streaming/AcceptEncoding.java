...
public class AcceptEncoding implements Comparable<AcceptEncoding>
{
   ...
   * @throws IllegalArgumentException if header value is not parsable.
   */
  public static List<AcceptEncoding> parseAcceptEncodingHeader(String headerValue, Set<EncodingType> supportedEncodings)
  {
    ...
    for(String entry : entries)
    {
      ...
      {
        throw new IllegalArgumentException(CompressionConstants.ILLEGAL_FORMAT + headerValue);
      }
      ...
      if (type != null && supportedEncodings.contains(type))
      {
        if (content.length > 1)
        {
          ...
          if (acceptEncodingPart.startsWith(CompressionConstants.QUALITY_PREFIX))
          {
            ...
            {
              throw new IllegalArgumentException(CompressionConstants.ILLEGAL_FORMAT + headerValue, e);
            }
          }
          else
          {
